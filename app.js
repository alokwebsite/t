...
// Set initial window size on launch
try {
    window.resizeTo(480, 460);
} catch(e) {}

document.getElementById('btn-open-settings').addEventListener('click', () => {
    try { window.resizeTo(580, 760); } catch(e) {}
...
document.getElementById('btn-cancel').addEventListener('click', () => {
    try { window.resizeTo(480, 460); } catch(e) {}
...
function createRow(binName, extsStr, color, isTypeRule = false, isDeletable = true, isIgnoreRule = false) {
    ...
    header.onclick = (e) => {
        ...
        if (!isIgnoreRule && e.target.classList && e.target.classList.contains('clip-color-picker')) return;
    ...
    let extNotice = document.createElement('div');
    extNotice.style.marginTop = '6px';
    extNotice.style.fontSize = '11px';
    extNotice.style.color = 'var(--text-dim)';
    extNotice.innerHTML = `🔒 Edit & Add extensions Available only in Auto file organizer pro <span style="font-size:10px;">(You are in a trial)</span>`;
    extGroup.appendChild(extNotice);
    
    let prefNotice = document.createElement('div');
    prefNotice.style.marginTop = '4px';
    prefNotice.style.fontSize = '11px';
    prefNotice.style.color = 'var(--text-dim)';
    prefNotice.innerHTML = `🏷️ Prefix sorting Available only in Auto file organizer pro <span style="font-size:10px;">(You are in a trial)</span>`;
    extGroup.appendChild(prefNotice);
    
    if(binName.includes("Video")) {
        let camNotice = document.createElement('div');
        camNotice.style.marginTop = '10px';
        camNotice.style.padding = '8px 10px';
        camNotice.style.background = 'rgba(255, 209, 102, 0.08)';
        camNotice.style.border = '1px solid rgba(255, 209, 102, 0.25)';
        camNotice.style.borderRadius = '6px';
        camNotice.style.fontSize = '12px';
        camNotice.style.color = '#FFD166';
        camNotice.style.fontWeight = '500';
        camNotice.style.lineHeight = '1.4';
        camNotice.innerHTML = `📷 Camera sorted folder wise Available only in Auto file organizer pro <span style="color:var(--text-dim); font-size:11px;">(You are in a trial)</span>`;
        extGroup.appendChild(camNotice);
    }
    
    if(binName.includes("Timelines")) {
        let tlNotice = document.createElement('div');
        tlNotice.style.marginTop = '10px';
        tlNotice.style.padding = '8px 10px';
        tlNotice.style.background = 'rgba(255, 209, 102, 0.08)';
        tlNotice.style.border = '1px solid rgba(255, 209, 102, 0.25)';
        tlNotice.style.borderRadius = '6px';
        tlNotice.style.fontSize = '12px';
        tlNotice.style.color = '#FFD166';
        tlNotice.style.fontWeight = '500';
        tlNotice.style.lineHeight = '1.4';
        tlNotice.innerHTML = `🎬 Size, resolution & frame rate sorted Available only in Auto file organizer pro <span style="color:var(--text-dim); font-size:11px;">(You are in a trial)</span>`;
        extGroup.appendChild(tlNotice);
    }
    ...
    if(binName.includes("Audio")) {
        let audNotice = document.createElement('div');
        audNotice.style.marginTop = '10px';
        audNotice.style.padding = '8px 10px';
        audNotice.style.background = 'rgba(255, 209, 102, 0.08)';
        audNotice.style.border = '1px solid rgba(255, 209, 102, 0.25)';
        audNotice.style.borderRadius = '6px';
        audNotice.style.fontSize = '12px';
        audNotice.style.color = '#FFD166';
        audNotice.style.fontWeight = '500';
        audNotice.style.lineHeight = '1.4';
        audNotice.innerHTML = `🎵 SFX and Music sorted folder wise Available only in Auto file organizer pro <span style="color:var(--text-dim); font-size:11px;">(You are in a trial)</span>`;
        extGroup.appendChild(audNotice);
    }
...
function renderSettings() {
    ...
    deepToggle.disabled = true;
    deepSwitch.style.opacity = '0.5';
    deepSwitch.style.cursor = 'not-allowed';
    ...
    deepSwitchLbl.appendChild(document.createTextNode("Deep Scan Master Folders (Locked)"));
...
document.getElementById('btn-modal-confirm').addEventListener('click', () => {
    let resetColors = document.getElementById('reset-colors') ? document.getElementById('reset-colors').checked : false;
    let resetFolders = document.getElementById('reset-folders') ? document.getElementById('reset-folders').checked : false;
    let resetPaths = document.getElementById('reset-paths') ? document.getElementById('reset-paths').checked : false;
    let resetPrefixesElem = document.getElementById('reset-prefixes');
    let resetPrefixes = resetPrefixesElem ? resetPrefixesElem.checked : false;
...
async function runOrganize() {
    ...
    // Give DaVinci Resolve a moment to sync its internal database after moving clips
    await new Promise(r => setTimeout(r, 1000));

    if(opts.empty) {
        try { mp.SetCurrentFolder(root); } catch(e) {}
        ...
        function removeEmpty(folder, isRoot) {
            ...
            if (toDelete.length > 0) {
                toDelete.forEach(f => {
                    try { mp.DeleteFolders([f]); } catch(e) {}
                });
            }
        ...
        let rootToDelete = [];
        emptyCheck.forEach(s => {
            if (removeEmpty(s, false)) {
                rootToDelete.push(s);
            }
        });
        if (rootToDelete.length > 0) {
            rootToDelete.forEach(f => {
                try { mp.DeleteFolders([f]); } catch(e) {}
            });
        }
    }
